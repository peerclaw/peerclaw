...
## Phase 15: A2A Task 模型与 ACP 桥接

与更广泛的 Agent 生态系统实现协议级互操作。

- [ ] **A2A Task 模型集成** — 将 PeerClaw Envelope 的请求-响应映射到 A2A Task 生命周期状态
- [ ] **A2A HTTP 桥接** — 将 PeerClaw Agent 暴露为标准 A2A HTTP 端点（Agent Card 位于 `/.well-known/agent.json`）
- [ ] **ACP 桥接** — ndJSON/stdio 桥接，让 ACP 兼容的 Agent 加入 PeerClaw 网络
- [ ] **协议自动检测** — 入站连接自动识别为 A2A、MCP 或 ACP 并路由到相应适配器
