...
## Phase 9: Request-Response 与 Agent 协作原语

实现同步的 Agent 间任务委托 — 多 Agent 协作的基础。

- [ ] **SendRequest** — `Agent.SendRequest(ctx, env, timeout) → (*Envelope, error)`，基于 TraceID 的请求-响应关联
  - 待处理请求注册表（`map[traceID]chan *Envelope`）
  - `HandleIncomingEnvelope` 中按 TraceID + MessageType=response 匹配响应
  - 基于 Context 的超时与取消
- [ ] **Envelope 响应辅助方法** — `envelope.NewResponse(request, payload)` 构造器
- [ ] **广播** — `Agent.Broadcast(ctx, env, destinations) → []error` 扇出消息
- [ ] **A2A Task 生命周期映射** — 将 Envelope 的请求-响应映射到 A2A Task 状态（submitted → working → completed/failed）

## Phase 10: 入站 Handler 路由

让 Agent 能够服务来自其他 Agent 的请求，基于能力进行路由分发。

- [ ] **Handler 注册** — `Agent.Handle(capability, func(ctx, *Envelope) (*Envelope, error))` 模式
- [ ] **自动路由** — 入站 Envelope 按 metadata 中的 capability/action 字段分发
- [ ] **自动响应** — Handler 返回值自动包装为 response envelope 发回
- [ ] **Agent Card 自动生成** — 已注册的 handler 名称自动填充 Card.Capabilities
- [ ] **中间件支持** — 入站 handler 的前置/后置 hook（日志、认证、限速）

## Phase 11: 企业简化配置

降低企业内网部署门槛。

- [ ] **`agent.NewSimple()`** — 简化构造器（仅 Name、ServerURL、KeypairPath、Capabilities）
  - 自动配置：Serverless=false，无 Nostr，无 DHT，无 STUN/TURN
- [ ] **企业部署指南** — 单 peerclaw-server + 内网多 Agent 部署方案
- [ ] **预配置信任** — 批量导入已验证联系人，适用于受管环境

## Phase 12: Nostr Relay 邮箱（离线消息投递）

将 Nostr relay 从备用传输升级为加密邮箱，在保持 P2P 纯度的同时实现可靠的离线消息投递。

- [ ] **加密 inbox 事件** — 将 XChaCha20 加密的 Envelope 以 Nostr gift-wrapped 事件（NIP-59/NIP-44）发布
- [ ] **Inbox relay 配置** — Agent Profile 扩展 `inbox_relays` 字段（类似 NIP-65）
- [ ] **投递流程** — Send() 优先级：1) WebRTC 直连 → 2) 信令中继 → 3) Nostr inbox relay
- [ ] **Inbox 同步** — Agent 启动时查询 inbox relay 获取上次同步以来的事件
- [ ] **投递确认** — 通过同一通道发回加密的投递回执
- [ ] **本地 outbox 重试** — 发送方持久化未确认消息；指数退避重试
- [ ] **TTL 与清理** — 可配置消息过期时间（默认 7 天）；已投递事件自动清理
- [ ] **唤醒信令** — inbox 有待处理消息时通过 WebSocket / Webhook / Cron 发送轻量通知

## Phase 13: OpenClaw SKILL.md 集成

通过 SKILL.md 技能文件快速集成 OpenClaw 生态。

- [ ] **SKILL.md 编写** — Markdown 技能文件，教 OpenClaw 使用 PeerClaw CLI/API 进行发现、消息和联系人管理
- [ ] **CLI 增强** — 确保 SKILL.md 所需的所有操作都可通过 CLI 命令完成（收件箱检查、联系人审批等）
...
与三大主流 Agent 协议深度集成，让 PeerClaw 成为各生态的原生参与者。

### Phase 15a: MCP Server (`peerclaw-mcp`)

独立 MCP Server 二进制 — 任何 MCP Host（Claude Code、VS Code Copilot、Cursor 等）均可将 PeerClaw 作为工具提供者使用。

- [ ] **`peerclaw-mcp` stdio 二进制** — 使用 `github.com/modelcontextprotocol/go-sdk` 将 `agent/tools/` 封装为 MCP Server
- [ ] **Tool 注册** — 8 个 PeerClaw 工具注册完整 JSON Schema、描述和 MCP Tool Annotations（`readOnlyHint`、`destructiveHint`、`idempotentHint`）
- [ ] **Streamable HTTP 传输** — 可选 HTTP 模式（`--transport http --port 8081`）用于远程 MCP 托管，默认 stdio
- [ ] **Resource 暴露** — Agent Card、信任存储条目和声誉数据作为 MCP Resource 暴露（`resources/list`、`resources/read`）
- [ ] **Session 管理** — `initialize` 握手、`Mcp-Session-Id` header、能力协商
- [ ] **配置示例** — `claude_desktop_config.json` / VS Code `settings.json` 示例配置，开箱即用

### Phase 15b: A2A Task 模型与 HTTP 桥接
