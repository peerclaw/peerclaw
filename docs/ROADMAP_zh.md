...
- [ ] **注册时的持有证明（Proof-of-Possession）** — 要求 Agent 在 `POST /agents` 注册时签署 challenge，防止公钥抢注（攻击者在真正所有者之前注册其公钥）
- [ ] **前向保密 / 会话密钥轮换** — 在长期 P2P 会话中实现定期 X25519 密钥轮换；通过 HKDF 棘轮派生每消息密钥，使单个会话密钥泄露不会暴露过去或未来的流量
- [ ] **加密信任存储** — 对本地信任数据库进行静态加密（使用从 Agent Ed25519 种子派生密钥的 XChaCha20-Poly1305），在主机文件系统被入侵时保护信任决策
- [ ] **测试覆盖率目标** — 建立最低覆盖率：core 80%+，agent/server/cli 70%+；在 CI 流水线中添加覆盖率门禁

### 错误处理与 API 质量

- [ ] **统一错误类型** — 为每个模块定义结构化错误类型（`core/errors`、`agent/errors`、`server/errors`），包含错误码，替代原始 `fmt.Errorf` 字符串，实现跨模块边界的程序化错误处理
- [ ] **Card.Validate() 方法** — 为 `core/discovery.Card` 添加 `Validate() error` 方法，检查必填字段（ID、PublicKey、Endpoint）、密钥格式和端点 URL 有效性；在注册和发现流程中调用

### CLI 与开发者体验

- [ ] **Flag 一致性审计** — 统一所有 CLI 命令的 flag 名称：`--agent-id`（而非 `--id`/`--agent`）、`--server-url`（而非 `--server`/`--url`）、`--output`（而非 `--format`）；为重命名的 flag 添加弃用别名
- [ ] **Shell 补全** — 通过 `peerclaw completion <shell>` 生成和安装 bash、zsh、fish、PowerShell 的 shell 补全脚本

### 声誉透明度

- [ ] **公开声誉阈值** — 在代码常量和用户文档中记录 EWMA 分数区间（如 0.0–0.3 = 低、0.3–0.7 = 中、0.7–1.0 = 高），让 Agent 运营者了解信任分数如何影响路由
- [ ] **声誉排序与筛选** — 为 `GET /agents` 目录列表添加 `sort=reputation` 和 `min_reputation=<float>` 查询参数，使 Agent 可以只发现信任度足够高的对等节点

### 插件生态

- [ ] **插件整合** — 将 Go（`picoclaw-plugin`）和 TypeScript（`openclaw-plugin`）作为 Tier 1 支持的插件，提供完整测试覆盖和 CI；将 Rust WASM（`ironclaw-plugin`）和 Python（`nanobot-plugin`）转为社区维护状态，提供清晰的贡献指南
